...
<div id="profilePage" style="position: relative; z-index: 0;">
    <div class="profile-page">
        <div class="profile-header">
            {{-- 背景上传区域 --}}
            <form id="bgForm" action="{{ route('profile.updateImage') }}" method="POST" enctype="multipart/form-data"
            style="display: inline;">
            @csrf
            @method('PUT')
                <label for="backgroundInput" style="cursor: pointer;">
                    <img class="profile-background"
                    src="{{ Auth::user()->profile && Auth::user()->profile->background_image && Auth::user()->profile->background_image !== 'default-bg.jpeg'
                        ? asset('storage/images/bg/' . Auth::user()->profile->background_image) . '?v=' . time()
          ...
          {{-- 不再用 label 控制点击上传，改为模态触发器 --}}
          <img class="profile-avatar"
            id="triggerAvatarModal"
            src="{{ Auth::user()->profile && Auth::user()->profile->profile_picture && Auth::user()->profile->profile_picture !== 'defaultaaa.webp'
                ? asset('storage/images/avatar/' . Auth::user()->profile->profile_picture) . '?v=' . time()
   ...
   <div style="display: flex; gap: 20px;">
       ...
       <div style="flex: 1; text-align: center;">
           ...
           <img id="previewCurrentAvatar"
            src="{{ Auth::user()->profile && Auth::user()->profile->profile_picture && Auth::user()->profile->profile_picture !== 'defaultaaa.webp'
                ? asset('storage/images/avatar/' . Auth::user()->profile->profile_picture)
                : asset('images/defaultaaa.webp') }}"
            onerror="this.onerror=null; this.src='{{ asset('images/defaultaaa.webp') }}';"
       ...
       <div style="flex: 2; padding: 20px; background: linear-gradient(135deg, #c3dafe, #e9d8fd); border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        ...
        <div class="profile-info">
            ...
            <div id="signatureDisplay" onclick="editSignature()" style="cursor: pointer;">
                <p id="signatureText">{{ Auth::user()->profile->signature ?? '点击添加签名' }}</p>
            </div>

            <!-- 编辑签名 input，默认隐藏 -->
            <textarea id="signatureInput"
                      style="display:none; width: 100%; padding: 6px; border-radius: 6px; border: 1px solid #ccc;"
                      onblur="saveSignature()"
                      onkeydown="handleKeyDown(event)">{{ Auth::user()->profile->signature }}</textarea>
            {{-- <p>{{ Auth::user()->profile->signature ?? '没有签名' }}</p> --}}
        </div>
...
<script>
    ...
    document.getElementById('customAvatarBtn').addEventListener('click', function () {
    ...
    const previewImg = document.getElementById("previewCurrentAvatar");
    const avatarMain = document.getElementById("triggerAvatarModal");
    ...
    avatar.addEventListener('click', () => {
        ...
        avatar.style.boxShadow = '0 0 12px rgba(0, 123, 255, 0.5)'; // 可保留，也可交由动画控制
...
function editSignature() {
    ...
    function saveSignature() {
        const input = document.getElementById('signatureInput');
        const newSignature = input.value;
        ...
        .then(data => {
            if (data.success) {
                document.getElementById('signatureText').innerText = newSignature || '点击添加签名';
            } else {
                alert('更新失败');
            }
